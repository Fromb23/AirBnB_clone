#!/usr/bin/python3

"""Unit tests for the 'state' module.
"""

import os
import unittest
from models.engine.file_storage import FileStorage
from models.state import State
from models import storage
from datetime import datetime


class TestState(unnitest.TestCase):
    """test cases for the 'State' class."""


    def setUp(self):
        pass

    def tearDown(self):
        FileStorage._FileStorage__objects = {}
        if os.path.exists(FileStorage._FileStorage__file_path):
        os.remove(FileStorage._FileStorage__file_path)

    def test_params(self):
         s = State()
         s3 = State("hello", "wait", "in")

         k = f"{type(s1).__name__}.{s1.id}"
         self.assertIsInstance(s.name, str)
         self.assertEqual(s3.name, "")
         
         #test for updating the name
         s.name = "Miami"
         self.assertEqual(s.name, "Miami")
         self.assertIn(k, storage.all())

    def test_init(self):
        """Test method for public instances"""
        s = State()
        s2 = State(**s.to_dict())
        self.assertIsInstance(s.id, str)
        self.assertIsInstance(s.created_at, datetime)
        self.assertIsInstance(s.updated_at, datetime)
        elf.assertEqual(s.updated_at, s2.updated_at)


    def test_str(self):
        """Test method for str representation"""
        s = State()
        string = f"[{type(s).__name__}] ({s.id}) {s.__dict__}"
        self.assertEqual(s.__str__(), string)

    def test_save(self):
        """Test method for save"""
        s = State()
        old_update = s.updated_at
        s.save()
        self.assertNotEqual(s.updated_at, old_update)

    def test_todict(self):
        """Test method for dict"""
        s = State()
        s2 = State(**s1.to_dict())
        a_dict = s2.to_dict()
        self.assertIsInstance(a_dict, dict)
        self.assertEqual(a_dict['__class__'], type(s2).__name__)
        self.assertIn('created_at', a_dict.keys())
        self.assertIn('updated_at', a_dict.keys())
        self.assertNotEqual(s, s2)


if __name__ == "__main__":
    unittest.main()
